...
# def _make_layer_normalization_hook(norm_type: str = "layernorm"):
#     """An internal function implementing the DeepLIFT correction for LayerNorm and RMSNorm.
...
#         # Inverse std: v = (σ^2 + eps)^{-1/2}. `torch.nn.RMSNorm.eps` defaults to
#         # None, which `F.rms_norm` interprets as the dtype's epsilon; mirror that
#         # here so the hook matches the forward pass it is correcting.
...
def _softmax(module, grad_input, grad_output):
    ...
    # TODO why this choice of c ("c is a constant for numerical stability")?
    c = torch.maximum(
