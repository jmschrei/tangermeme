...
--------------
	...
	- Adds ``integrated_gradients_op``, a factory returning a rule for any single-input module that has no closed form. The multiplier it implements integrates the module's Jacobian along the straight-line path from the reference activation to the observed one, which is the integrated-gradients construction applied to one layer rather than to a whole model. The integral is numerically approximated using Gauss-Legendre quadrature, and the procedure computes only vector-Jacobian products rather than materializing the full Jacobian. Note that it does not approximate the closed-form rules. A path integral and the rescale secant coincide to floating-point noise for an elementwise function, but not for one whose outputs couple across positions, where the two differ by orders of magnitude more and raising ``K`` does not close the gap. Both satisfy summation-to-delta; neither is a more accurate version of the other.
